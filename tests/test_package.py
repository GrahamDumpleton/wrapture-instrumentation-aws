...
import wrapture_instrumentation_aws
...
def test_version_info_is_a_tuple_of_strings() -> None:
    assert isinstance(wrapture_instrumentation_aws.__version_info__, tuple)
    assert all(
        isinstance(part, str) for part in wrapture_instrumentation_aws.__version_info__
    )
    assert len(wrapture_instrumentation_aws.__version_info__) in (3, 4)


def test_version_is_formatted_from_version_info() -> None:
    parts = wrapture_instrumentation_aws.__version_info__
    base = ".".join(parts[:3])
    ...
    assert wrapture_instrumentation_aws.__version__ == expected
...
def test_version_is_pep_440_compliant() -> None:
    ...
    assert re.match(pattern, wrapture_instrumentation_aws.__version__)


def test_version_matches_the_installed_distribution() -> None:
    assert metadata.version(DISTRIBUTION) == wrapture_instrumentation_aws.__version__
...
def test_importing_the_package_imports_nothing_else() -> None:
    ...
    assert appeared.strip() == "['wrapture_instrumentation_aws']"
...
def test_each_entry_point_loads_an_instrumentation_named_for_its_target(
    ...
    # `name = "botocore"` and the class's `target` agree.
...
def test_the_listing_tool_reports_every_entry_cleanly() -> None:
    ...
    version = wrapture_instrumentation_aws.__version__
...
def test_the_readme_table_lists_every_target() -> None:
    ...
    for point in ENTRY_POINTS:
        ...
        link = (
            "https://github.com/GrahamDumpleton/wrapture-instrumentation-aws"
